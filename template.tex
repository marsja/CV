...
\fancyfoot[CO,CE]{$if(cletter)$ Cover Letter $else$ CV including publications $endif$ - Erik Marsja Page - \thepage \hspace{1pt} of \pageref{LastPage}}
...
$if(phone)$$if(fontawesome)$ \faPhone \hspace{1 mm}$else$\emph{Phone:}$endif$  $phone$  \hspace{1 mm} $endif$ &  & $if(email)$$if(fontawesome)$\faEnvelopeO \hspace{1 mm}$else$\emph{E-mail:}$endif$ \href{mailto:}{\tt $email$} \hspace{1 mm}$endif$  \\
 ...
 \multicolumn{3}{c}{$if(lang)$\emph{Languages: }$lang$$endif$}
 \\\hline
\end{tabularx}
\end{table}
